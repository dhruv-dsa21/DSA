...
//     }
  int dfs(int node,int prev,int cost,vector<bool> &apple){
      int childrencost = 0;
      for(auto it:g[node]){
          if(it==prev){
              continue;
          }
          childrencost+=dfs(it,node,2,apple);
      }
      if(childrencost==0&&apple[node]==false){
          return 0;
      }
    ...
    int minTime(int n, vector<vector<int>>& edges, vector<bool>& hasApple) {
      ...
      return dfs(0,-1, 0, hasApple); // cost of reaching the root is 0. For all others, its 2.
    }
};
